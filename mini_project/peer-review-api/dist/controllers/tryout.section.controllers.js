...
class TryoutSectionController {
    static async getAllTryoutSections(req, res) {
        try {
    ...
    static async getTryoutSectionById(req, res) {
        try {
            const { tryoutSectionId } = req.params;
            const tryoutSection = await TryoutSectionServices.getTryouSectiontById(tryoutSectionId);
            if (!tryoutSection) {
                res.status(404).json({
                    success: false,
