...
const updateReviewValidator = async (req, res, next) => {
    ...
    if (existingReviewObject.referenceId.trim() !== "") {
        const error = ReviewValidator.updateReplyReviewValidator.validate(req.body).error;
        if (error) {
            res.status(400).json({
                success: false,
        ...
        const error = ReviewValidator.updateParentReviewValidator.validate(req.body).error;
        if (error) {
            res.status(400).json({
                success: false,
                message: error.message,
...
const createReviewValidator = async (req, res, next) => {
    ...
    if (referenceId.trim() !== "") {
        ...
        const error = ReviewValidator.parentReviewValidator.validate(req.body).error;
        if (error) {
            res.status(400).json({
                success: false,
                message: error.message,
...
const alreadyExistParentReviewValidator = async (req, res, next) => {
    const { referenceId, courseId, tryoutId, type } = req.body;
    const { userId } = req.params;
    if (referenceId.trim() === "") {
        if (courseId) {
